function remplirComparateur() {
  const options = athletesDuSport().map((athlete) => {
...
function afficherComparaison() {
  ...
  const bouton = document.querySelector("#chartButton");
  bouton.addEventListener("click", afficherOuCacherCamembert);
}

function ligneTableau(cle, valeurA, valeurB) {
  return `
    <tr>
      <td>${libelle(cle)}</td>
      <td>${valeurA}</td>
...
function camembert(athleteA, athleteB, cles) {
